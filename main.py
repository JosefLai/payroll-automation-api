...
# Vacation / Holiday are no longer simply dropped - they count toward the
# weekly 44h threshold using their actual QBO-recorded hours (a full day is
# typically 8h, but genuine partial days - e.g. 5.5h - are preserved as-is,
# not overridden). They contribute no OT themselves, and get removed from
# the final billable output since they're not job-costed.
...
def calculate_alberta_ot(df):
    ...
    # 包含 jobcode_2 欄位預處理
    # NOTE: fillna("") BEFORE astype(str) - otherwise genuinely blank cells
    # (e.g. Lunch break / Holiday rows with no class or service item) get
    # rendered as the literal text "nan" instead of staying blank.
    ...
    # ----------------------------------------------------
    # Rule 1b: Vacation / Holiday - 計入每週 44 小時加班門檻計算,
    # 使用 QBO 實際記錄的時數 (不強制覆寫成 8 小時 - 部分請假日的實際時數,
    # 例如 5.5 或 2.0 小時, 是刻意輸入的真實半天請假, 不應被覆寫成整天)。
    # 本身不產生加班費, 且最終不會出現在可請款的輸出中
    # (僅適用於一般技師; Admin/overhead 員工已在上一步被完全排除)
    # ----------------------------------------------------
    ...
    # ----------------------------------------------------
    # Rule 1b (cont.): Vacation/Holiday rows have now correctly consumed
    # their share of the daily/weekly OT thresholds above - drop them from
    # the final billable output since they are not job-costed.
    # ----------------------------------------------------
